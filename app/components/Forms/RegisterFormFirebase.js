...
import { CheckboxRedux, SelectRedux } from './ReduxFormMUI';
...
const username = value => (
  ...
  alphanumeric
  */
  value && !/^(?=.{8})[a-z]{1,1}[a-z\d]*_?[a-z\d]+$/.test(value)
    // value && !/^[a-z0-9]{8,}$/.test(value)
    ? 'Minimum 8 characters containing lowercase letters(a-z), numbers(0-9), and underscore'
    : undefined
);
            ...
            <div>
              <FormControl className={classes.formControl}>
                <Field
                  ...
                  placeholder={intl.formatMessage(messages.loginFieldUserType)}
                  label={intl.formatMessage(messages.loginFieldUserType)}
                  required
                  className={classes.field}
                  validate={[required, userType]}
                >
                  <MenuItem value={userTypes.lab}>{userTypes.lab}</MenuItem>
                  <MenuItem value={userTypes.industry}>{userTypes.industry}</MenuItem>
                  <MenuItem value={userTypes.student}>{userTypes.student}</MenuItem>
                  <MenuItem selected value={userTypes.personal}>{userTypes.personal}</MenuItem>
                </Field>
              </FormControl>
            </div>
            <div>
