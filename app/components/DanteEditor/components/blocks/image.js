...
export default class ImageBlock extends React.Component {
  ...
  uploadCompleted(url) {
    ...
    // Update again the width, height, and aspect_ratio states.
    // Solve the image size not correct when the image is resized.
    this.img = new Image()
    this.img.src = this.image_tag.src
    this.setState({ url })
    this.img.onload = () => {
      this.setState({
        width: this.img.width,
        height: this.img.height,
        aspect_ratio: this.getAspectRatio(this.img.width, this.img.height)
      }, this.updateData)
    }
