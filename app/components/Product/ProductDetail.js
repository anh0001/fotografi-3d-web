...
class ProductDetail extends React.Component { // eslint-disable-line
  ...
  render() {
    ...
    const itemAttr = (item) => {
      if (item !== undefined) {
        return {
          id: detailContent.getIn([productIndex, 'productId']),
          name: detailContent.getIn([productIndex, 'productName']),
          thumbnail: detailContent.getIn([productIndex, 'thumbnail']),
          price: detailContent.getIn([productIndex, 'productPrice']),
          quantity: qty
        };
      }
      return false;
        ...
        <AppBar className={classes.appBar}>
          <Toolbar>
            <Typography variant="h6" noWrap color="inherit" className={classes.flex}>
              {detailContent.getIn([productIndex, 'productName'])}
            </Typography>
              ...
              <section className={classes.detailWrap}>
                ...
                <div className={classes.price}>
                  <Typography variant="h5">
                    <span>
                      Rp.
                      {detailContent.getIn([productIndex, 'productPrice']).toLocaleString()}
                    </span>
                  </Typography>
                    ...
                    <Fragment>
                      <Typography variant="caption" component="h5">
                        <span className={Type.lineThrought}>
                          Rp.
                          {detailContent.getIn([productIndex, 'productPrevPrice']).toLocaleString()}
                        </span>
                      </Typography>
                      <Chip label={intl.formatMessage(messages.discount) + detailContent.getIn([productIndex, 'discount'])} className={classes.chipDiscount} />
                    </Fragment>
                  )}
                  {detailContent.getIn([productIndex, 'availableQty']) == 0 && (
                    <Chip label={intl.formatMessage(messages.sold_out)} className={classes.chipSold} />
                  )}
                </div>
                <div className={classes.ratting}>
                  <Rating value={detailContent.getIn([productIndex, 'rating'])} max={5} readOnly />
                </div>
                <Typography component="p" className={classes.desc}>
                  {detailContent.getIn([productIndex, 'productDescription'])}
                </Typography>
                {!(detailContent.getIn([productIndex, 'availableQty']) == 0) && (
