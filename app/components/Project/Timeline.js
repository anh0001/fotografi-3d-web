...
class Timeline extends React.Component {
  ...
  handleClickOpt = (event, curContent) => {
    this.setState({
      ...
      selectedProjectData: curContent,
    });
  };
  ...
  render() {
    ...
    const { anchorElOpt, openComment, selectedProjectData } = this.state;
    ...
    const getItem = dataArray => dataArray.map(data => {
      ...
      return (
        <li key={data.get('projectId')}>
          <div className={classes.iconBullet}>
            <Tooltip id={'tooltip-icon-' + data.get('projectId')} title={data.get('date') + ' ' + data.get('time')}>
          ...
          <Card className={classes.cardSocmed}>
            ...
            <div className={classes.rightIcon}>
              ...
              {(data.get('status') === projectStatus.draft) && (userType === userTypes.lab) ?
      ...
      <Fragment>
        ...
        >
          {listOptions.map(option => (
            <MenuItem key={option} onClick={(event) => this.handleCloseOpt(event, option, selectedProjectData)}>
              {option}
            </MenuItem>
          ))}
        </Menu>
