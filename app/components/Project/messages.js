/*
 * Dashboard and Widgets Messages
 *
 * This contains all the text for the Dashboard and Widgets page.
 */
import { defineMessages } from 'react-intl';

export const scope = 'boilerplate.components.Project';

export default defineMessages({
  placeholder: {
    id: `${scope}.WritePost.placeholder`,
    defaultMessage: 'What\'s on your mind?',
  },
  upload: {
    id: `${scope}.WritePost.upload`,
    defaultMessage: 'Upload Photo',
  },
  public: {
    id: `${scope}.WritePost.public`,
    defaultMessage: 'Public',
  },
  friends: {
    id: `${scope}.WritePost.friends`,
    defaultMessage: 'Friends',
  },
  only_me: {
    id: `${scope}.WritePost.only_me`,
    defaultMessage: 'Only Me',
  },
  post: {
    id: `${scope}.WritePost.post`,
    defaultMessage: 'Post',
  },
  comments: {
    id: `${scope}.comments`,
    defaultMessage: 'Comments',
  },
  write_comments: {
    id: `${scope}.comments.write_comments`,
    defaultMessage: 'Write Comment',
  },
  connection: {
    id: `${scope}.aside.connection`,
    defaultMessage: 'Connection',
  },
  my_profile: {
    id: `${scope}.aside.my_profile`,
    defaultMessage: 'My Profile',
  },
  people: {
    id: `${scope}.aside.people`,
    defaultMessage: 'People You May Know',
  },
  connect: {
    id: `${scope}.aside.connect`,
    defaultMessage: 'Connect',
  },
  mutual_connection: {
    id: `${scope}.aside.mutual_connection`,
    defaultMessage: 'Mutual Connection',
  },
  add: {
    id: `${scope}.add`,
    defaultMessage: 'Add',
  },
  cancel: {
    id: `${scope}.cancel`,
    defaultMessage: 'Cancel',
  },
  reset: {
    id: `${scope}.reset`,
    defaultMessage: 'Reset',
  },
  request: {
    id: `${scope}.request`,
    defaultMessage: 'Request',
  },
  see_all: {
    id: `${scope}.aside.see_all`,
    defaultMessage: 'See All',
  },
  title: {
    id: `${scope}.title`,
    defaultMessage: 'Title',
  },
  starting_date: {
    id: `${scope}.starting_date`,
    defaultMessage: 'Starting Date',
  },
  ending_date: {
    id: `${scope}.ending_date`,
    defaultMessage: 'Ending Date',
  },
  currency: {
    id: `${scope}.currency`,
    defaultMessage: 'Currency',
  },
  industrial_partners: {
    id: `${scope}.industrial_partners`,
    defaultMessage: 'Industrial Partners',
  },
  total_funding: {
    id: `${scope}.total_funding`,
    defaultMessage: 'Total Funding',
  },
  trend: {
    id: `${scope}.aside.trend`,
    defaultMessage: 'Trends For You',
  },
  open_add_new_project_tooltip: {
    id: `${scope}.open_add_new_project_tooltip`,
    defaultMessage: 'Add New Project',
  },
  add_new_project_form_title: {
    id: `${scope}.add_new_project_form_title`,
    defaultMessage: 'New Project',
  },
});
